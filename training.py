...
class Trainer:
    ...
    def predict(self,data_loader):
        self.model.load_state_dict(torch.load(f'{args.save_path}/{args.embedding_type}_{args.mapping_strategy}_{args.privatization_strategy}_eps_{args.eps}_save_{args.save_stop_words}_model.pkl'))
        self.model.eval()
        y_list = []
        y_hat_list = []
        for batch in tqdm(data_loader):
    ...
    def save_model(self):
        if not os.path.exists(args.save_path):
            os.mkdir(args.save_path)
        torch.save(self.model.state_dict(),f'{args.save_path}/{args.embedding_type}_{args.mapping_strategy}_{args.privatization_strategy}_eps_{args.eps}_save_{args.save_stop_words}_model.pkl')
